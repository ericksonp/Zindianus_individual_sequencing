...
final<-foreach(pop=pops, .errorhandling="remove")%do%{
  ...
  p.data<-fread(paste0("/scratch/perickso/private/ind_seq/popgen/smcpp/", pop, "/", pop, "_cv_10folds_8knots_10000gen_scaf4/", pop, "_CV10fold_final_plot.csv"))
  p.data[,pop:=pop]
  return(p.data)
}
...
folds<-foreach(pop=pops, .errorhandling="remove")%do%{
  ...
  folds2<-foreach(i=c(0:30), .errorhandling="remove")%do%{
    p.data<-fread(paste0("/scratch/perickso/private/ind_seq/popgen/smcpp/", pop, "/", pop, "_cv_10folds_8knots_10000gen_scaf4/fold", i, "/", pop, "_fold", i, "_final_plot.csv"))
    p.data[,fold:=i]
    p.data[,pop:=pop]
    return(p.data)
...
geno <- snpgdsOpen("/scratch/perickso/private/ind_seq/popgen/zaprionus.individual.nosingleton.2023.withID.mac3.removeinv.vcf.gz.gds" , allow.fork=T)
...
k.all<-ggplot(kinship.table[group.pop1%in%groups &group.pop1==group.pop2], aes(x=k0, y=kinship, fill=group.pop1))+
  geom_point(color="white", pch=21, size=4, alpha=0.75)+
  labs(fill=NULL)+
  labs(x="IBD0")+
  scale_fill_manual(values = c(friendly_pal("ito_seven"), friendly_pal("bright_seven")[c(3,4,7)]))+
  theme(legend.position="right")+
  guides(fill=guide_legend(nrow=10,byrow=T, override.aes = list(size = 2)))+
  theme(legend.text=element_text(size=8))

#kinship over time
k.time<-ggplot(kinship.table[Location.pop1=="VA-CM"&Location.pop2=="VA-CM"], aes(x=time.diff, y=kinship, fill=kinship.group))+
  geom_point(color="white", pch=21, size=4, alpha=0.75)+
  labs(x="Days between sampling", fill=NULL)+
  scale_fill_manual(values = friendly_pal("ito_seven"))+
  theme(legend.position="right")+
  guides(fill=guide_legend(nrow=5, byrow=T, override.aes = list(size = 2)))+
  theme(legend.text=element_text(size=8))
...
jpeg("/scratch/perickso/private/ind_seq/Figures/kinship_vs_k0_bypop_noinv.jpeg", height=6, width=8, res=300, units="in")
ggplot(kinship.table[group.pop1%in%groups &group.pop1==group.pop2], aes(x=k0, y=kinship, color=kinship.group))+
  ...
  scale_color_manual(values = friendly_pal("ito_seven")[2:5])+
