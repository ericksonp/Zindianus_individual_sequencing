...
#make a figure of LD 
#ggplot(ld.win)+geom_point(aes(x=start, y=max.ld))+facet_grid(CHR~.)
ld.win[,window.ld.snps:=ifelse(nsnps<5,F,T)]
ld.win[,inv:=rleid(window.ld.snps)]
#ld.win[,inv:=rleid(nsnps>5)]
#ld.win[,inv2:=nsnps>5]
#ld.win[,inv.nsnps:=rleid(nsnps>5)]


ld.inv.sum<-ld.win[,.(inv.start=min(start), inv.end=max(end)), .(CHR, inv, window.ld.snps)]
...
largeinv<-ld.inv.sum[window.ld.snps==T&length>1000000]
bed<-data.table(chr=paste0("Scaffold_", largeinv$CHR),
                start=largeinv$inv.start-1,
                stop=largeinv$inv.end-1)

write.table(bed, "/scratch/perickso/private/ind_seq/popgen/LDdecay_inversions_snpmethod.bed", quote=F, sep="\t", col.names = F, row.names = F)



#make a figure

inv<-fread("/scratch/perickso/private/ind_seq/popgen/LDdecay_inversions_snpmethod.bed", header=F)
...
b<-ggplot(ld.win)+geom_rect(data=inv, aes(xmin=start, xmax=stop, ymin=-Inf, ymax=Inf),fill="grey90")+
  geom_point(aes(x=start, y=as.integer(window.ld.snps)))+
...
jpeg("/scratch/perickso/private/ind_seq/popgen/plots/LD_figure_snp_with_smoove.jpg", height=10, width=8,units="in", res=300 )
