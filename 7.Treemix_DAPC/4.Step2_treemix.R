...
library(OptM)
library(plyr)
source("/scratch/perickso/private/ind_seq/popgen/scripts/treemix_functions.R") #path to required functions for this analysis

########################################################
############# (A) Test migration events ################
########################################################

folder <- file.path(path="/scratch/perickso/private/ind_seq/popgen/treemix/test_migrations")                     #path to files of TreeMix replicates with different migration edges (m) to test
...
setwd("/scratch/perickso/private/ind_seq/popgen/treemix//final_runs")                                             #folder with all TreeMix outputs from the final runs
maxLL("5pops10snps_1m_finalrun_", nt=30)                                         #first argument is stem of TreeMix output files, nt = number of runs
...
cfTrees("5pops10snps_1m_finalrun_", nt=30, p=1, m='PH85')                        #m is the method to calculate pairwise distances between unique trees (default is Robinson-Foulds distance)
...
GetMigrStats(input_stem="5pops5ksnps_3m_finalrun_", nt=30)                       #arguments as above, writes file "MS_and_stats.txt" into current directory
...
setwd("/scratch/perickso/private/ind_seq/popgen/treemix//final_runs/bootstraps")
