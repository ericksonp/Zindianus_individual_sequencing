...
#' 
#' @export
plot_treemix <- function(obj, plot.nodes = TRUE, plot.migration = TRUE, branch.colour = "grey", branch.width = 0.5, label = TRUE, xmax = NA, ...) {
  ...
  if (plot.migration && nrow(subset(stuff$edges, type == "MIG"))) {
    p <- p + ggplot2::geom_curve(data = subset(stuff$edges, type == "MIG"),
      ...
      viridis::scale_color_viridis("migration weight")
  }
  
  if (label)
    p <- p + ggplot2::geom_text(ggplot2::aes(x = xo, y = y, label = pop), hjust=0)+ggplot2::coord_cartesian(clip = "off")
