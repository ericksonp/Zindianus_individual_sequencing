...
b<-ggplot(pixy.dxy[pop1!="other"&pop2!="other"][chromosome=="Scaffold_2"][window.mid>26590000&window.mid<26700000])+
...
c<-ggplot(pixy.pi[pop!="other"][chromosome=="Scaffold_2"][window.mid>26590000&window.mid<26700000])+
  geom_line(aes(x=window.mid, 
                y=avg_pi, 
                color=pop, 
                group=pop))+
...
d<-ggplot(thetas[chrom==2][WinCenter>26590000&WinCenter<26700000])+
...
e<-ggplot(depth.sum[chr==2&win>26590000&win<26700000&label%in%(c("Africa", "FL", "VA"))], 
       (aes(x=win/1000000, 
            y=mean.depth,
...
jpeg("/scratch/perickso/private/ind_seq/popgen/plots/diversity_stats_scaffold_2_cpr.jpg", 
     height=10, 
     width=8,
     units="in", 
     res=300 )
