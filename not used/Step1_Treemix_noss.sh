...
if [ $outgroup = "noRoot" ]; then
	dowork() {
		...
		b=1`date +%N`
		let "c=$a+$b"
		treemix -i $2 -bootstrap -k $3 -se -noss -seed $c -o "bootstrap/"$4"_constree_bootrep_"$1
	}
	export -f dowork
	...
	dowork() {
		...
		b=1`date +%N`
		let "c=$a+$b"
		treemix -i $2 -bootstrap -k $3 -se -noss -root $5 -seed $c -o "bootstrap/"$4"_constree_bootrep_"$1
	}
	export -f dowork
