...
bp.99<-quantile(bp.va.fl$M_XtX, 0.99, na.rm=T)
bp.va.fl[,q99:=M_XtX>=bp.99]
...
setnames(bp.va.fl, "CHR", "chr")
setnames(bp.va.fl, "q99", "bpq99")

#merge
alldata<-merge(z, ihs.VAall, by=c("chr", "pos" , "old.snp.id"), all=T)
alldata<-merge(alldata, bp.va.fl, by=c("chr", "pos", "old.snp.id"), all=T)
...
window.sum<-foreach(window=wins$index, .combine="rbind", 
                .errorhandling="remove")%dopar%{
                  ...
                  bp.tmp<-bp.va.fl[chr==paste0("Scaffold_", wins[J(window),CHR]) & pos>=wins[J(window), start] & pos<wins[J(window),end]]
...
write.csv(window.sum, file="/scratch/perickso/private/ind_seq/popgen/ihs_bp_fst_1kbwindows_VA_ALL.csv")
...
write.csv(snptags, file="/scratch/perickso/private/ind_seq/popgen/ihs_bp_fst_1kbwindows_snpstoplot_VA_ALL.csv")
