...
wgscan<-foreach(i=c(1:5), .errorhandling="remove")%do% {
    print(i)
        ...
        if(i==3){
        haps.to.use<-samps[(assigned_sex=="F"|(assigned_sex=="M"&hapnum==1))&(continent=="Africa"), haplotype.id] #can add other conditionals here for specific subsets 
        }
        else{
          haps.to.use<-samps[continent=="Africa", haplotype.id] #can add other conditionals here for specific subsets 
        }
...
save(wgscan.ihs, file=paste0("/scratch/perickso/private/ind_seq/popgen/rehh_wgscan_bychr_Africa.Rdat"))
