...
wgscan<-foreach(i=c(1:5), .errorhandling="remove")%do% {
    print(i)
        ...
        if(i==3){
        haps.to.use<-samps[(assigned_sex=="F"|(assigned_sex=="M"&hapnum==1))&continent=="NorthAmerica", haplotype.id] #can add other conditionals here for specific subsets 
        }
        else{
          haps.to.use<-samps[continent=="NorthAmerica", haplotype.id] #can add other conditionals here for specific subsets 
        }
    
        hh_subset = subset(hh, select.hap = haps.to.use, min_perc_geno.mrk = 90, min_maf=0)
        scan <- scan_hh(hh_subset, discard_integration_at_border = FALSE, polarized=T)
...
save(wgscan.ihs, file=paste0("/scratch/perickso/private/ind_seq/popgen/rehh_wgscan_bychr_NorthAmerica.Rdat"))
